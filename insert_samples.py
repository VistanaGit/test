...
# Assuming Exhibition model and SessionLocal are already defined in your setup

def insert_exhibitions():
    session = SessionLocal()  # Assuming SessionLocal is defined in your setup
    try:
        # Possible names and descriptions for the exhibitions
        names = [
            "Modern Art Expo",
            "Tech Innovation Fair",
            "Cultural Heritage Exhibit",
            "Science and Discovery Festival",
        ...
        # Shuffle names to ensure unique selection
        random.shuffle(names)

        exhibitions = []

        for i in range(5):  # Insert exactly 5 exhibition records with unique names
            name = names[i]
        ...
        session.commit()
        print("Exhibitions inserted successfully!")
